...
class _PaymentScreenState extends State<PaymentScreen> {
  ...
  void initState() {
    ...
    _finalAmount = widget.meeting.price;
    _loadCoupons();
  }
  ...
  Future<void> _loadCoupons() async {
    ...
    try {
      ...
      _availableCoupons = _availableCoupons
          .where((coupon) => coupon.isApplicableTo(widget.meeting.price))
          .toList();
  ...
  void _calculateFinalAmount() {
    ...
    _finalAmount = widget.meeting.price - discount;
  ...
  Future<void> _processPayment() async {
    ...
    try {
      // 임시 예약 생성
      final reservation = ReservationModel.fromUserAndEvent(
        userId: widget.user.id,
        userName: widget.user.name,
        userPhone: widget.user.phone,
        userGender: widget.user.gender,
        userAgeGroup: widget.user.ageGroup,
      ...
      // Firestore에 예약 저장
      _reservationId =
          await _firestoreService.createReservationBeforePayment(reservation);

      if (_finalAmount > 0) {
        // 토스페이 결제 URL 생성
        final paymentUrl = _createTossPaymentUrl(
          orderId: _reservationId!,
          orderName: widget.meeting.title,
          amount: _finalAmount,
  ...
  Future<void> _completePayment() async {
    ...
    try {
      // 결제 완료 처리
      await _firestoreService.completeReservationAfterPayment(
        _reservationId!,
            ...
            Row(
              ...
              children: [
                ...
                ClipRRect(
                  ...
                  child: widget.meeting.imageUrls.isNotEmpty
                      ? (widget.meeting.imageUrls.first.startsWith('assets/')
                          ? Image.asset(
                              widget.meeting.imageUrls.first,
                              width: 80,
                              height: 80,
                              fit: BoxFit.cover,
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        '일시: ${DateFormat('yyyy.MM.dd (E) HH:mm', 'ko_KR').format(widget.meeting.scheduledDate)}',
                        style: TextStyle(
                          fontSize: 14,
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              ...
              children: [
                const Text('모임 참가비'),
                Text('${NumberFormat('#,###').format(widget.meeting.price)}원'),
              ],
            ),
